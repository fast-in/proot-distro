pretty_name="Ubuntu 16.04 LTS (Xenial Xerus)"
distro="ubuntu16.04-xenial"
...
if ! [ -f '$tarball' -a -r '$tarball' ]; then
	...
	curl --location --insecure --fail \
	     --output "$tarball" \
	     --url "http://cdimage.ubuntu.com/ubuntu-base/releases/16.04/release/ubuntu-base-16.04.6-base-$archurl.tar.gz"
	echo
fi
