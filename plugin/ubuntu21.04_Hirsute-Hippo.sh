pretty_name="Ubuntu 21.04 (Hirsute Hippo)"
distro="ubuntu21.04-hirsute"
...
if ! [ -f '$tarball' -a -r '$tarball' ]; then
	...
	curl --location --insecure --fail \
	     --output "$tarball" \
	     --url "http://cdimage.ubuntu.com/ubuntu-base/releases/21.04/release/ubuntu-base-21.04-base-$archurl.tar.gz"
	echo
fi
