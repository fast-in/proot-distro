pretty_name="Ubuntu 14.04 LTS (Trusty Tahr)"
distro="ubuntu14.04-trusty"
...
if ! [ -f '$tarball' -a -r '$tarball' ]; then
	...
	curl --location --insecure --fail \
	     --output "$tarball" \
	     --url "http://cdimage.ubuntu.com/ubuntu-base/releases/14.04.4/release/ubuntu-base-14.04-core-$archurl.tar.gz"
	echo
fi
